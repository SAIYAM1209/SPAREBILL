...
export default function App() {
  ...
  }, [customers, staff, inventory, invoices, settings]);

  useEffect(() => {
    if (customers.length > 0 || staff.length > 0 || inventory.length > 0) {
      saveAll();
    }
  }, [customers, staff, inventory, invoices, settings, saveAll]);
  ...
  return (
    <>
      ...
      <div className="app-container">
        <Sidebar role={role} currentPage={currentPage} onNavigate={setCurrentPage} onLogout={logout} customer={currentCustomer} invoices={invoices} />
        <main className="main">
          {role === 'admin' && currentPage === 'dashboard' && (
            <Dashboard customers={customers} staff={staff} inventory={inventory} invoices={invoices} onNavigate={setCurrentPage} settings={settings} />
          )}
          {((role === 'admin' || role === 'cashier') && currentPage === 'billing') && (
            <Billing customers={customers} inventory={inventory} setInventory={setInventory} invoices={invoices} setInvoices={setInvoices} setCustomers={setCustomers} showToast={showToast} settings={settings} />
          )}
          ...
          {role === 'customer' && (
            <CustomerPanel inventory={inventory} setInventory={setInventory} invoices={invoices} setInvoices={setInvoices} showToast={showToast} currentCustomer={currentCustomer} />
          )}
        </main>
      </div>
      <Toast toast={toast} />
...
function Sidebar({ role, currentPage, onNavigate, onLogout, customer, invoices }) {
  const navItems = [
...
function Dashboard({ customers, staff, inventory, invoices, onNavigate, settings }) {
  ...
  return (
    <div id="dashboard" className="page fade-in">
      ...
      <div className="section">
        <h3>🚨 Live Alerts</h3>
        {lowStockItems.length > 0 ? lowStockItems.map(p => (<div key={p.id} className="card danger" style={{ marginBottom: 10 }}>🔴 Low stock: <b>{p.name}</b> ({p.stock} left)</div>)) : <div className="card success">✅ No low stock items</div>}
      </div>
      <div className="section">
...
function Billing({ customers, inventory, setInventory, invoices, setInvoices, setCustomers, showToast, settings }) {
  const [billCustomer, setBillCustomer] = useState('');
  const [billPhone, setBillPhone] = useState('');
  const [billDate, setBillDate] = useState(new Date().toISOString().slice(0, 10));
  const [billInvoiceNo] = useState('INV-' + Date.now());
  const [billItems, setBillItems] = useState([{ category: '', part: '', brand: '', price: 0, qty: 1 }]);
  ...
  const addBillRow = () => { setBillItems([...billItems, { category: '', part: '', brand: '', price: 0, qty: 1 }]); };
  ...
  const generateInvoice = () => {
    ...
    setBillCustomer(''); setBillPhone(''); setBillItems([{ category: '', part: '', brand: '', price: 0, qty: 1 }]); setBillDiscount(0);
    ...
    <div id="billing" className="page fade-in">
      ...
      <div className="section">
        ...
        <h3>Items</h3>
          ...
          <tbody>
            {billItems.map((item, index) => (
              <tr key={index}>
                <td>
                  <select value={item.category} onChange={(e) => handlePartChange(index, 'category', e.target.value)}>
                    <option value="">Select</option>
                    {getCategories().map(cat => <option key={cat} value={cat}>{cat}</option>)}
                  </select>
                </td>
                <td>
                  <select value={item.part} onChange={(e) => handlePartChange(index, 'part', e.target.value)} disabled={!item.category}>
                    <option value="">Select</option>
                    {getParts(item.category).map(p => <option key={p} value={p}>{p}</option>)}
                  </select>
                </td>
                <td>
                  <select value={item.brand} onChange={(e) => handlePartChange(index, 'brand', e.target.value)} disabled={!item.category || !item.part}>
                    <option value="">Select</option>
                    {getBrands(item.category, item.part).map(b => <option key={b} value={b}>{b}</option>)}
                  </select>
                </td>
        ...
        <div style={{ marginTop: 20, display: 'flex', gap: 10 }}>
          ...
          <button className="btn gray" onClick={() => { setBillCustomer(''); setBillPhone(''); setBillItems([{ category: '', part: '', brand: '', price: 0, qty: 1 }]); setBillDiscount(0); }}>Clear</button>
        </div>
      </div>
    </div>
  );
...
function Inventory({ inventory, setInventory, showToast, invoices }) {
  ...
  const addPart = () => { if (!newPart.category || !newPart.name || !newPart.brand || !newPart.price) { showToast('Fill all required fields', 'error'); return; } setInventory([...inventory, { ...newPart, id: Date.now() }]); showToast('Part added', 'success'); setNewPart({ category: '', name: '', brand: '', price: 0, stock: 0, threshold: 5 }); setShowModal(false); };
  const openEdit = (index) => { setEditIndex(index); setEditPart({ ...inventory[index] }); };
  const saveEdit = () => {
    if (!editPart.category || !editPart.name || !editPart.brand || !editPart.price) { showToast('Fill all required fields', 'error'); return; }
    setInventory(inventory.map((p, i) => (i === editIndex ? { ...p, ...editPart } : p)));
    setEditIndex(null);
    showToast('Part updated', 'success');
  };
  const deletePart = (index) => { if (window.confirm('Delete part?')) { setInventory(inventory.filter((_, i) => i !== index)); showToast('Part deleted', 'warn'); } };
...
function CustomerPanel({ inventory, setInventory, invoices, setInvoices, showToast, currentCustomer }) {
    ...
    <div id="customer" className="page fade-in">
      ...
      <div id="customer-catalog" className="section">
        <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
          <input placeholder="Search products..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ flex: 1 }} />
